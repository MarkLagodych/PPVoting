 ...
 *    и положить в папку PPVoting/bin/voter с названиями voter_1.bin и *
 *    voter_2.bin.                                                     *
 ...
 * WiFi-сети ESP8266 не может прошиться без замыкания.
 * FLASH_WITH_JUMPER_ONLY определяет, будет ли модуль прошиваться
 * исключительно с замыкающей перемычкой, или стоит задержаться при 
 * включении чтобы решить проблему.
 */
...
const unsigned char                    // Последняя часть статического IP-адреса.
   myIp = 2;                           // Менять только значение этой переменной!
