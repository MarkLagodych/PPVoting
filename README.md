...
**Голосовалке для записи настроек нужно нажать на среднюю кнопку.** Если при включении (или после перезагрузки) нажата средняя кнопка, голосовалка переходит в режим настройки. Если Вы используете схему из `src/voter/scheme`, для того, чтобы подключить и прошить голосовалку, посмотрите  "Примечания: компиляция и сборка" > "Печатная плата".
